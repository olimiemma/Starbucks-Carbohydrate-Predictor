...
const StarbucksCarbs = () => {
  // Manage input and result as separate state values
  const [inputCalories, setInputCalories] = useState(300);
  ...
  const handleSubmit = (e) => {
    ...
    const caloriesNum = Number(inputCalories);
    
    // Validation checks
    if (inputCalories === '' || inputCalories === null || inputCalories === undefined) {
    ...
    const carbsValue = predictCarbs(caloriesNum);
    const categoryValue = getCarbCategory(carbsValue);
    ...
    setError('');
    console.log('Updated result to:', caloriesNum, carbsValue, categoryValue);
  };
